...
| MYS-VAL-01 | A regra de documento especial limpa todos os erros e força resultado válido, sem distinguir CPF de RG | `VALDOCS.NSN#L174-L179` | Pode aprovar cadastro com inconsistências documentais | ALTA |
| MYS-VAL-02 | `TITULO` e `CTPS` são coletados na entrada, mas não participam de nenhuma validação observável | `VALDOCS.NSN#L52` | Regras documentais podem estar incompletas ou dispersas | ALTA |
| MYS-VAL-03 | Lista de prefixos especiais contém códigos aparentemente administrativos/teste sem documentação funcional | `VALDOCS.NSN#L41` | Exceções indevidas podem ser preservadas ou perdidas na migração | MÉDIA |
...
### MYS-VAL-01: Documento especial sobrescreve erros acumulados

- **Arquivo**: `01-arqueologia/legado-sifap/natural-programs/VALDOCS.NSN#L174-L179`
- **Trecho de código**:
...
- **O que o código faz**: força o resultado para válido e apaga todos os erros acumulados.
- **Hipótese do time**: workaround histórico para casos especiais de cadastro.
- **Risco se ignorarmos**: aprovação incorreta de beneficiários sem critério explícito.

---

### MYS-VAL-02: `TITULO` e `CTPS` sem validação visível
...
- **Hipótese do time**: a validação pode existir em outro programa ou ter sido removida.
- **Risco se ignorarmos**: perda de requisitos de conformidade documental na modernização.

---

### MYS-VAL-03: Prefixos especiais sem documentação funcional
...
- **O que o código faz**: mantém lista fixa de exceções sem contexto de negócio no próprio código.
- **Hipótese do time**: códigos administrativos/teste herdados de homologação.
- **Risco se ignorarmos**: comportamento divergente entre ambientes e perda de cenários especiais legítimos.
...
- Total de mistérios encontrados: **25**
- Mistérios adicionais fora do Par 2: **3** (`MYS-VAL-01`, `MYS-VAL-02`, `MYS-VAL-03`)
- Confiança alta: **12** (MYS-PGT-01, 02, 03, 04, 05 · MYS-CON-02, 03, 05, 08 · MYS-REL-01 · MYS-VAL-01, 02)
- Confiança média: **9** (MYS-PGT-06, 07 · MYS-CON-01, 04, 06 · MYS-REL-02, 03, 05 · MYS-VAL-03)
